...
class TagObjects(object):
    def __init__(self, fps=7):
        ...
        rospy.init_node("learning_objects", anonymous=True)
    ...
    def run(self):
        while not rospy.is_shutdown() and self._created:
            #Get image sections
            sections = self.splitImageIntoColourSections()
    ...
    def splitImageIntoColourSections(self, use_depth=False):
        if use_depth:
            ...
            if self.isValid(self.color_frame, self.color_config):
                #Use median blur to remove speckles
                color_frame = cv2.medianBlur(self.color_frame, 5)
